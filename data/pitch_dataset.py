...
PITCH_VOCAB_IDX_TOK = {
    i:k for i,k in enumerate(PITCH_GLOSSARY.keys())
}

PITCH_VOCAB_TOK_IDX = {
    v:k for k,v in PITCH_VOCAB_IDX_TOK.items()
}

RESULT_VOCAB_IDX_TOK = {
    i:k for i,k in enumerate(RESULT_GLOSSARY.keys())
}

RESULT_VOCAB_TOK_IDX = {
    v:k for k,v in RESULT_VOCAB_IDX_TOK.items()
}
...
class PitchBaseDataset(Dataset):
    ...
    def __init__(self, statcast_data):
        assert statcast_data.index.name == 'at_bat_id', "statcast_data must have at_bat_id as index"
        self.data = statcast_data

    def __getitem__(self, x):
        ab = self.data.loc[x]
        pitches = torch.tensor(ab.pitch_type.map(PITCH_VOCAB_TOK_IDX).to_list(), dtype=torch.long)
        results = torch.tensor(ab.type.map(RESULT_VOCAB_TOK_IDX).to_list(), dtype=torch.long)
